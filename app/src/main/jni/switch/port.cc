...
void SwPort::packet_handler(u_char *user, const struct pcap_pkthdr *pkthdr, const u_char *packet) {
    modify_ip_packet(packet);
    ...
    ++Datapath::buffer_id;
    (*(pcap_dp->dp))->pkt_buffer.insert(
            std::pair<uint32_t, struct packet *>(Datapath::buffer_id, pkt));
